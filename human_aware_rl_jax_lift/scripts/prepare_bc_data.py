...
def _featurize_worker(worker_id, df, jax_layout: str, terrain, use_path_cost: bool = False, tf_compat: bool = False) -> tuple[list, list]:
    """
    ...
    mlp = get_mlp_for_layout(jax_layout) if use_path_cost else None
    if traj is None:
        return [], []
    ...
    for obs_ep, acts_ep in zip(ep_obs, ep_acts):
        for state, joint_act in zip(obs_ep, acts_ep):
            try:
                ...
                feats_p0, feats_p1 = featurize_state_64(terrain, jax_state, mlp=mlp, tf_compat=tf_compat)
...
def featurize_layout_split(df, jax_layout: str, split_label: str, terrain, use_path_cost: bool = False, tf_compat: bool = False) -> dict:
    ...
    for worker_id in workers:
        feats, acts = _featurize_worker(worker_id, layout_df, jax_layout, terrain, use_path_cost, tf_compat)
        if not feats:
            skipped += 1
            continue
    ...
    expected_dim = 62 if tf_compat else 64
    assert features_arr.shape[1] == expected_dim, (
        f"Expected {expected_dim}-dim features (tf_compat={tf_compat}), got {features_arr.shape}."
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--use_path_cost",
        action="store_true",
        help="Use graph path cost for closest-feature selection. Default: Manhattan.",
    )
    parser.add_argument(
        "--tf_compat",
    ...
    for jax_layout in args.layouts:
        ...
        payload = {
            "train": featurize_layout_split(df_train, jax_layout, "train", terrain, args.use_path_cost, args.tf_compat),
            "test":  featurize_layout_split(df_test,  jax_layout, "test",  terrain, args.use_path_cost, args.tf_compat),
        }
