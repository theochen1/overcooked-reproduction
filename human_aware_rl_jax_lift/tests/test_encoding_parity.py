...
from human_aware_rl_jax_lift.encoding.bc_features import featurize_state_64, get_mlp_for_layout
...
def test_bc_features_match_legacy():
    """BC features with path-cost MLP: verify structure and path-cost integration.

    JAX returns 64-dim (includes abs_pos); legacy returns 62-dim (layout-dependent).
    We verify: (1) JAX produces 64-dim when mlp is passed, (2) path-cost mode runs
    without error, (3) feature ranges are plausible. Exact numeric match with legacy
    is not asserted due to layout parsing / coordinate convention differences.
    """
    mdp, terrain, Action, mdp_mod = _get_mdp_and_terrain("simple")
    from overcooked_ai_py.mdp.actions import Direction
    mlp = get_mlp_for_layout("simple")

    state = mdp.get_standard_start_state()
    jax_state = from_legacy_state(terrain, state)

    # With mlp: path-cost mode, 62-dim output when tf_compat=True (facing_empty_counter excluded)
    jax_f0, jax_f1 = featurize_state_64(terrain, jax_state, mlp=mlp, tf_compat=True)
    assert jax_f0.shape[0] == 62 and jax_f1.shape[0] == 62
    assert np.all(np.isfinite(np.asarray(jax_f0))) and np.all(np.isfinite(np.asarray(jax_f1)))

    # Without mlp: Manhattan mode, same structure
    jax_f0_m, jax_f1_m = featurize_state_64(terrain, jax_state, mlp=None, tf_compat=True)
    assert jax_f0_m.shape[0] == 62 and jax_f1_m.shape[0] == 62

    # Legacy returns 62-dim when tf_compat=True (facing_empty_counter excluded)
    legacy_f0, legacy_f1 = mdp.featurize_state(state, mlp)
    assert legacy_f0.shape[0] == 62 and legacy_f1.shape[0] == 62

    # After a few steps: path-cost still works
    actions = [
        (Direction.NORTH, Action.STAY),
        (Action.INTERACT, Action.STAY),
        (Direction.SOUTH, Action.STAY),
        (Direction.EAST, Action.STAY),
        (Action.INTERACT, Action.STAY),
    ]
    for a in actions:
    ...
    jax_state = from_legacy_state(terrain, state)
    jax_f0, jax_f1 = featurize_state_64(terrain, jax_state, mlp=mlp, tf_compat=True)
    assert jax_f0.shape[0] == 62 and np.all(np.isfinite(np.asarray(jax_f0)))
