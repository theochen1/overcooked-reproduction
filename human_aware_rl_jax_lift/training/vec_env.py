...
def _encode_single(terrain: Terrain, state: OvercookedState, agent_idx: jnp.ndarray, *, tf_compat: bool = True):
    """Encode one state; returns (obs_training_agent, obs_other_agent)."""
    p0_obs, p1_obs = lossless_state_encoding_20(terrain, state, tf_compat=tf_compat)
...
@functools.partial(jax.jit, static_argnames=('tf_compat',))
def batched_step(
    terrain: Terrain,
    ...
    *,
    ...
    obs0, obs1 = jax.vmap(
        lambda s, idx: _encode_single(terrain, s, idx, tf_compat=tf_compat)
...
def encode_obs(terrain: Terrain, bstate: BatchedEnvState, *, tf_compat: bool = True) -> Tuple[jnp.ndarray, jnp.ndarray]:
