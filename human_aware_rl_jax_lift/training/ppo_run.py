...
- Saves a separate `best_after_sp_anneal/` checkpoint that only competes once
...
from human_aware_rl_jax_lift.agents.ppo.train import PPOConfig, compute_gae, create_train_state, ppo_update_step
...
def ppo_run(
    ...
    # Build path-cost distance table for BC partner featurization (matches eval).
    # Only needed for BC-partner runs; keep None for SP so compilation path is simpler.
    bc_dist_table = None
    if bc_params is not None:
        from human_aware_rl_jax_lift.encoding.bc_features import build_dist_table, get_mlp_for_layout
        print(f"[{_ts(t0)}] Building BC dist_table for path-cost featurization...")
        sys.stdout.flush()
        mlp = get_mlp_for_layout(layout_name)
        bc_dist_table = build_dist_table(mlp, terrain)
        print(f"[{_ts(t0)}] dist_table ready  shape={bc_dist_table.shape}")
    ...
    for seed in seeds:
        ...
        probe_obs0, _ = encode_obs(terrain, probe_bstate, tf_compat=tf_compat)
        ...
        # When tf_compat=True, all envs share the same agent_idx per rollout.
        tf_agent_idx_rng = None
        if tf_compat:
            tf_agent_idx_rng = np.random.RandomState(0)
            idx_val = tf_agent_idx_rng.randint(0, 2)
            bstate = bstate.replace(
                agent_idx=jnp.full(config.num_envs, idx_val, dtype=jnp.int32)
            )

        obs0, _ = encode_obs(terrain, bstate, tf_compat=tf_compat)
        ...
        # Threshold prevents saving a "best" model at trivially low reward.
        save_best_thresh = 50.0 if tf_compat else 0.0
        ...
        for update in range(num_updates):
            ...
            # Synchronized agent_idx when tf_compat is enabled.
            if tf_agent_idx_rng is not None:
                idx_val = tf_agent_idx_rng.randint(0, 2)
                bstate = bstate.replace(
                    agent_idx=jnp.full(config.num_envs, idx_val, dtype=jnp.int32)
                )
            ...
            if ep_sparse > best_sparse and ep_sparse > save_best_thresh:
