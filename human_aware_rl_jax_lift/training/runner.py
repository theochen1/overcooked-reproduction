...
BC partner uses an "unstuck" heuristic (stuck_time=3): when the partner repeats the
same position AND orientation for stuck_time+1 steps, mask the last stuck_time
...
def _init_bc_partner_state(num_envs: int):
    ...
    or_hist = jnp.zeros((num_envs, 4), dtype=jnp.int32)
...
def _unstuck_adjust_probs(
    ...
    *,
    ...
    """Vectorized unstuck heuristic: masks recently-repeated actions when partner is stationary.

    Stuck = same (pos, orientation) for stuck_time+1 steps. When stuck, block
    only the last stuck_time actions (not all 4). Skip adjustment if all four
    movement directions would be blocked.
    """
    if stuck_time <= 0:
        return probs
    ...
    # Stuck: pos_and_or identical for stuck_time+1 values.
    p0 = pos_hist[:, 0, :]
    o0 = or_hist[:, 0]
    same_pos = jnp.all(pos_hist == p0[:, None, :], axis=(1, 2))
    same_or = jnp.all(or_hist == o0[:, None], axis=1)
    apply = ready & same_pos & same_or

    # Block only last stuck_time actions.
    # act_hist indices [1,2,3] = the 3 most recent when buffer is full.
    last_n = act_hist[:, -stuck_time:]  # [N, stuck_time]
    one_hot = jax.nn.one_hot(last_n, probs.shape[-1], dtype=jnp.float32)  # [N, stuck_time, A]
    ...
    # If all movement actions blocked, skip adjustment.
    movement_probs = jnp.sum(out[:, :4], axis=-1, keepdims=True)
    skip = movement_probs <= 0.0
    renormed = jnp.where(norm > 0.0, out / norm, probs)
    result = jnp.where(skip, probs, renormed)
    return jnp.where(apply[:, None], result, probs)
...
def _push_history(
    ...
    *,
    ...
    """Append (pos, or, act) into fixed-size ring buffers when update_mask is True."""
    pos_shifted = jnp.concatenate([pos_hist[:, 1:, :], pos_xy[:, None, :]], axis=1)
    or_shifted = jnp.concatenate([or_hist[:, 1:], or_val[:, None]], axis=1)
...
def make_rollout_fn(
    ...
    *,
    ...
    bc_stuck_time: int = 3,
    dist_table: Optional[jnp.ndarray] = None,
    tf_compat: bool = False,
):
    ...
    bc_stuck_time:
      Unstuck heuristic parameter (default 3).
    """
    ...
    if bc_params is None:
        ...
        def _rollout(
            ...
            obs0_init, obs1_init = encode_obs(terrain, bstate, tf_compat=tf_compat)
            obs0_init = obs0_f
        ...
        def _rollout(
            ...
            def scan_step(carry, _):
                ...
                # ---- Other-agent BC (batched features + unstuck heuristic) ------
                if dist_table is not None:
                    f0, f1 = jax.vmap(
                        lambda s: featurize_state_64(terrain, s, dist_table=dist_table, tf_compat=tf_compat)
                    )(bstate.states)
                else:
                    f0, f1 = jax.vmap(
                        lambda s: featurize_state_64(terrain, s, tf_compat=tf_compat),
                    )(bstate.states)
                ...
                probs_bc = _unstuck_adjust_probs(
                    probs_bc, pos_hist, or_hist, act_hist, hist_len, stuck_time=int(bc_stuck_time)
                )
                ...
                or_val = bstate.states.player_or[jnp.arange(num_envs), other_idx]   # [N]
                ...
                transition = (obs0, actions, rewards, dones, values, logp, sparse_r)
                new_carry = (new_bstate, new_obs0, new_obs1, rng, pos_hist, or_hist, act_hist, hist_len, use_sp_mask)
            ...
            init_carry = (bstate, obs0_init, obs1_init, rng, pos_hist0, or_hist0, act_hist0, hist_len0, use_sp0)
            (final_bstate, final_obs0, _final_obs1, _rng, _ph, _oh, _ah, _hl, _us), transitions = jax.lax.scan(
                scan_step,
                init=init_carry,
                xs=None,
                length=horizon,
