...
class OvercookedGridworld(object):
    ...
    def get_lossless_state_encoding_shape_legacy(self):
        """Return shape for legacy 20-channel encoding (matching 2019 paper)."""
        return np.array(list(self.shape) + [20])

    def lossless_state_encoding_legacy(self, overcooked_state, debug=False):
        """
        Legacy 20-channel state encoding matching the original 2019 paper implementation.
        
        This encoding is required for reproducing the original paper results.
        It does NOT include tomato support, urgency features, or the new soup state representation.
        
        Channels (20 total):
        - 2 player location layers
        - 8 player orientation layers (4 directions x 2 players)
        ...
        """
        assert self.num_players == 2, "Legacy encoding only supports 2 players"
        assert type(debug) is bool
        
        # Original paper features (NO tomato, NO urgency)
        ...
        def process_for_player(primary_agent_idx):
            ...
            # OBJECT & STATE LAYERS (legacy format)
            for obj in all_objects:
                if obj.name == "soup":
                    # Legacy soup handling - uses old (soup_type, num_onions, cook_time) format
                    # or new format with ingredients list
                    if hasattr(obj, 'ingredients'):
                        # New format - count onions from ingredients
                        num_onions = obj.ingredients.count("onion") if hasattr(obj.ingredients, 'count') else len([i for i in obj.ingredients if i == "onion"])
                        # Determine cook_time based on new object attributes
                        # CRITICAL: _cooking_tick starts at -1 for partial soups (not yet cooking)
                        # The original encoding used cook_time >= 0, so we must clamp to 0
                        if hasattr(obj, '_cooking_tick'):
                            cook_time = max(0, obj._cooking_tick) if obj._cooking_tick is not None else 0
                        else:
                            cook_time = 0
                    else:
                        # Old format - direct state tuple
                    ...
                    if obj.position in self.get_pot_locations():
                        ...
                        # If soup is not in a pot (being carried), put it in separate mask
                    ...
                    # Ignore tomatoes in legacy encoding
                    pass
                else:
                    # Skip unrecognized objects instead of raising error
                    pass
            ...
            state_mask_stack = np.transpose(state_mask_stack, (1, 2, 0))
            assert state_mask_stack.shape[:2] == self.shape
            assert state_mask_stack.shape[2] == len(LAYERS), f"Expected 20 channels, got {state_mask_stack.shape[2]}"
            return np.array(state_mask_stack).astype(int)
